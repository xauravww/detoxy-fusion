...
function Chat() {
  const { sendMessage, messages, setMessages ,message } =
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      setLoading(true);
      try {
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/message`, {
      ...
      try {
        if (input.startsWith("/imagine ")) {
          ...
          const imageResponse = await axios.post(
            `${import.meta.env.VITE_BACKEND_URL}/generateImage`,
            { prompt, settings }
          );
  ...
  return (
    <div
    className="flex max-w-screen w-full bg-gradient-to-r from-gray-800 via-gray-900 to-black h-[calc(100vh-11vh)] md:h-[calc(100vh-11vh)] lg:h-[calc(100vh-10vh)]"
  >
      <div
        className={`sidebar ${
  ...
  {filteredMessages
    ...
    .map((message) => {
      ...
      return (
        <div
          key={message.id}
          className={`flex mb-4 ${(isCurrentUser && message.type!="image") ? "justify-end" : "justify-start"}`}
        >
          <div
            ...
            {message.imageUrl &&  (
              <div className="relative">
                ...
                {isCurrentUser && (
                  <>
                    <img
                      className="absolute -right-20 top-24 rounded-md h-10 w-10"
                      onClick={() => handleForwardImage(message)}
                      src="/assets/forward.svg"
                      style={{ filter: whiteColorFilter }}
                    />
                    <img
                      className="absolute -right-20 top-44 rounded-md h-10 w-10"
